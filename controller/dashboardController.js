    ...
    try {
        ...
        const locationsWithScores = allLocations.map(loc => {
            ...
            if (scores.length > 0) {
                ...
                currentScore = scores.reduce((sum, score) => sum + score, 0) / scores.length;
            }
...
export const getTodaysActivities = async (req, res) => {
    ...
    try {
        ...
        const roleFilter = await RBACFilterService.getLocationFilter(user, 'cleaneractivity');
        ...
        console.log(cleanerReviewWhere, "cleaner review where clause todays activities")
        ...
        const [cleanerActivities, userReviews] = await Promise.all([
            prisma.cleaner_review.findMany({
                ...
                select: {
                    ...
                    created_at: true,
                    updated_at: true,
                    cleaner_user: { // ✅ Correct relation name from schema
                        select: { id: true, name: true }
                    },
                    location: { // ✅ Correct relation name from schema
                        select: { id: true, name: true }
                    }
                },
            ...
            user.role_id <= 3 ? prisma.user_review_qr.findMany({
        ...
        cleanerActivities.forEach(activity => {
            ...
            if (activity.status === 'completed' && activity.updated_at && activity.updated_at > activity.created_at) {
                activities.push({
